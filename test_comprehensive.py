...
# ══════════════════════════════════════════════════════════════════════════════
# ТЕСТ 6: /update_week — сохраняет существующие записи клиентов
# ══════════════════════════════════════════════════════════════════════════════

def test_6_update_week_preserves_bookings():
    sep("ТЕСТ 6: /update_week сохраняет записи клиентов")
    db = fresh_db()
    ...
    # Администратор меняет расписание: убирает «11:00» из new_times, добавляет «13:00»
    ...
    # 11:00 занят → должен остаться, несмотря на отсутствие в new_times
    assert "11:00" in result['preserved_bookings'], \
        f"Занятый слот 11:00 должен быть сохранён: {result}"
    print(f"✅ Занятый слот 11:00 сохранён (preserved={result['preserved_bookings']})")
